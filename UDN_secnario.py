...
class scenario():
    ...
    def show_connect(self, adj, activate, rate):
        ...
        for flag, ue in enumerate(activate):
            if rate[flag]>0.1:
                plt.plot([self.AP_loc["x"][adj[flag]], self.UE_loc["x"][ue]],[self.AP_loc["y"][adj[flag]], self.UE_loc["y"][ue]], linewidth=2, linestyle=":")
                plt.text(self.UE_loc["x"][ue],  self.UE_loc["y"][ue], str(np.round(rate[flag],1)), fontsize=6)
